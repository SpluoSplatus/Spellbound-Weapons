#execute as @e[distance=..2.500,type=!#1_splatus:spellbound/misc] at @s run summon minecraft:snowball ~ ~3 ~ {Motion:[0.0,-10.0,0.0],Tags:["creatures_nonvariant","spellbound_damagewithuuid_snowball"],LeftOwner:1b,Item:{id:"minecraft:air",Count:1b}}
...
execute if entity @s[tag=spellbound_endlight_arrow_flamed] as @e[distance=..2.500,type=!#1_splatus:spellbound/misc] run data merge entity @s {Fire:60s}
...
execute as @e[distance=..0.0001,type=!#1_splatus:spellbound/misc] run damage @s 1 1_splatus:spellbound/projectile by @p[tag=spellbound_temp5]
execute as @e[distance=0.0001..2.500,type=!#1_splatus:spellbound/misc] run damage @s 5 1_splatus:spellbound/projectile by @p[tag=spellbound_temp5]
